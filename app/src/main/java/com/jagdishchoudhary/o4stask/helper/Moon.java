 ...
 *
 * Algorithms from "Mathnomy on the Personal Computer" by Oliver Montenbruck
...
{
    /*
     * Example (from United States Naval Observatory):
     *	For 1996.11.02 (San Francisco), Moonset 12:33, Moonrise 23:26
     */
    public static final double	MOONRISE				= Math.sin(+8.0 / 60.0);
     ...
     *							double[SET] is the local civil time of sunset
     * Note the following special result values:
     *	ABOVE_HORIZON		The moon does not set
     *	BELOW_HORIZON		The moon does not rise
     * Note that these are independent: there are days each month when the
     * moon rises and does not set (and vice-versa).
     */
    public static double[] riseSet(
            Date			javaDate,
            long			timezoneOffset,
            double			latitude,
            double			longitude
...
//            Format.log("longitude",	longitude);
        ...
        double yMinus			= sinAltitude(
                DATE, 0.0, longitude, cosLatitude, sinLatitude
        ) - MOONRISE;
        ...
        for (double hour = 1.0; hour <= 24.0; hour += 2.0) {
            double yThis		= sinAltitude(
                    DATE, hour, longitude, cosLatitude, sinLatitude
            ) - MOONRISE;
            double yPlus		= sinAltitude(
                    DATE, hour + 1.0, longitude, cosLatitude, sinLatitude
            ) - MOONRISE;
     ...
     * computation.
     */
    protected static double sinAltitude(
            double				MJD0,
            double				hour,
            double				longitude,
            double				cosLatitude,
    ...
    )
    {
        ...
        double result	= sinLatitude * Math.sin(moon[Astro.DEC])
                + cosLatitude * Math.cos(moon[Astro.DEC]) * Math.cos(TAU);
        return (result);
    }
    /**
     ...
     * Note: this is a little messy - the problem is the moon takes slightly
     * more than 24 hours to orbit the earth, hence there are days that have
     * no moonrise or moonset. The U.S. Naval Observatory web page displays
     * times for tomorrow or yesterday to fill in the blanks.
     */
    ...
    public static String riseSetString(double			moonrise, double			moonset)
    {
        if (Astro.isEvent(moonrise)) {
            if (Astro.isEvent(moonset)) {
                if (moonrise <= moonset) {
                    return (
                            getRiseSetString(moonrise, "Moonrise at ")
                                    + getRiseSetString(moonset, "; moonset at ")
                    );
                }
                else {
                    return (
     ...
     */
    public static String getRiseSetString(double		eventValue, String		whichEvent	/* "Moonrise" or "Moonset"	*/)
    {
